...
from app.jobs.controller import (
    add_JOB,
    get_jobs,
    get_job_by_id,
    get_del_job_by_id,
...
def handle_add_multiple_jobs():
    print("add multiple jobs")
    try:
        data = request.json
        # Ensure the incoming data is a list of job dictionaries
        if not isinstance(data, list):
            return (
                jsonify({"error": "Data must be a list of job objects", "status": 400}),
                400,
            )
        ...
        return (
            jsonify({"error": f"Failed to add multiple jobs: {e}", "status": 500}),
            500,
        )
